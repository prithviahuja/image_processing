...
cap.set(3,1500)
cap.set(4,840)    # set dimention of the camera screen
model=YOLO("best20epochs.pt")
classNames = [
    'BIODEGRADABLE', 'CARDBOARD', 'GLASS', 'METAL', 'PAPER', 'PLASTIC'
]
#predifined class names used in garbage daatset on which yolo is trained
...
while True:       ## use while loop(inifinite loop) until the window is closed to capture continuoes images
    ...
    for r in results:
        ...
        for box in boxes:
            ...
            cvzone.putTextRect(img,f'{classNames[cls]}{conf}',(x1,y1-20),scale=2,thickness=1)#put the confidence level along with classname from given dataset and its position,scale,thickness
